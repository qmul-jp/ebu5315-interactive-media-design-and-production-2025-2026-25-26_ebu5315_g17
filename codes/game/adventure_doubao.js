...
const langData = {
    cn: {
        ...
        question4: "问题4：请你移动点A,B,C,D，使得角ABC和角ADC为直角，并且劣弧AB等于劣弧CD",
    ...
    en: {
        ...
        question4: "Question 4: Move points A,B,C,D to make ∠ABC and ∠ADC right angles, and minor arc AB equals minor arc CD",
        finalChallenge: "Prepare for the Final Challenge!",
        confirm: "Confirm",
        warriorHp: "❤️ HP: ",
        warriorAtk: " | ⚔️ ATK: ",
        enemyHp: "💀 HP: ",
        win: "Enemy defeated! Next Level?",
        winFinal: "Victory!",
...
function initCanvas() {
    ...
    game.canvasSize = 308;
    game.canvas.width = game.canvasSize;
    game.canvas.height = game.canvasSize;
...
function updateCircleCenter() {
    ...
    circlePoints.O = { x: center, y: center };
    circlePoints.radius = radius;
    
    circle2Points.O = { x: center, y: center };
    circle2Points.radius = radius;
    
    circle3Points.O = { x: center, y: center };
    circle3Points.radius = radius;
    
    circle4Points.O = { x: center, y: center };
    circle4Points.radius = radius;
}
...
function drawCircle4() {
    ...
    const ctx = game.ctx;
    ctx.clearRect(0,0,game.canvas.width,game.canvas.height);
    const { O, A, B, C, D, radius } = circle4Points;
    
    ctx.beginPath(); ctx.arc(O.x,O.y,radius,0,Math.PI*2); 
    ctx.strokeStyle="#000"; ctx.lineWidth=2; ctx.stroke();
    
    ctx.setLineDash([5,5]);
    ctx.beginPath(); ctx.moveTo(O.x-radius,O.y); ctx.lineTo(O.x+radius,O.y); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(O.x,O.y-radius); ctx.lineTo(O.x,O.y+radius); ctx.stroke();
    ctx.setLineDash([]);
    ...
    ctx.beginPath(); ctx.moveTo(D.x,D.y); ctx.lineTo(A.x,A.y); ctx.stroke();
    
    drawPoint(O, "O", "#000", circle4Points.pointSize);
    drawPoint(A, "A", "#000", circle4Points.pointSize);
    drawPoint(B, "B", "#000", circle4Points.pointSize);
    drawPoint(C, "C", "#000", circle4Points.pointSize);
    drawPoint(D, "D", "#000", circle4Points.pointSize);
}
...
function checkCircle4() {
    const { A, B, C, D, O } = circle4Points;
    const rightAngleABC = isRightAngle(A, B, C);
    const rightAngleADC = isRightAngle(A, D, C);
    const angleAOB = calcAngle(O, A, B);
    const angleCOD = calcAngle(O, C, D);
    const minorArcAB = angleAOB <= 180 ? angleAOB : 360 - angleAOB;
    const minorArcCD = angleCOD <= 180 ? angleCOD : 360 - angleCOD;
    const largerArc = Math.max(minorArcAB, minorArcCD);
    const smallerArc = Math.min(minorArcAB, minorArcCD);
    const relativeError = (largerArc - smallerArc) / largerArc;
    const arcEqual = relativeError < 0.10;
    return rightAngleABC && rightAngleADC && arcEqual;
}
...
function startDragCircle4(mouse) {
    if (isPointClicked(mouse, circle4Points.A)) game.draggingPoint = circle4Points.A;
    if (isPointClicked(mouse, circle4Points.B)) game.draggingPoint = circle4Points.B;
    if (isPointClicked(mouse, circle4Points.C)) game.draggingPoint = circle4Points.C;
    if (isPointClicked(mouse, circle4Points.D)) game.draggingPoint = circle4Points.D;
}
function startDrag(e) {
    ...
    if(game.currentQuestionIndex === 3) startDragCircle4(mouse);
}
function drag(e) {
    ...
    if (game.currentQuestionIndex === 3) {
        lockToCircle(game.draggingPoint, circle4Points);
        adsorbToPoints(game.draggingPoint, circle4Points.adsorptionPoints, circle4Points.adsorbDistance);
        drawCircle4();
    }
}
function stopDrag() {
...
function adsorbPoint(point) {
    ...
    for(let t of targets){ 
        if(Math.hypot(point.x-t.x, point.y-t.y) < adsorbDistance){ 
            point.x = t.x; point.y = t.y; break; 
        } 
    }
}
...
function updateLanguageUI() {
    ...
    langBtns.forEach(btn => btn.textContent = langData[lang].lang);
    newGameBtn.textContent = langData[lang].newGame;
    continueBtn.textContent = langData[lang].continue;
    backBtn.textContent = langData[lang].back;
    confirmBtn.textContent = langData[lang].confirm;
    warriorImg.alt = langData[lang].warrior;
    enemyImg.alt = langData[lang].enemy;
    navBtns.forEach(btn => btn.title = langData[lang][btn.dataset.tipKey]);
    updateStatus();
    if (!gameScreen.classList.contains('hidden')) {
        if (game.currentQuestionIndex === 3) {
            questionText.textContent = langData[lang].question4;
        } else if (game.currentQuestionIndex < questions.length) {
            questionText.textContent = langData[lang][`question${game.currentQuestionIndex + 1}`];
        }
    }
    if (!resultModal.classList.contains('hidden')) {
        if (game.isWin) {
            resultText.textContent = game.currentQuestionIndex === 3 ? langData[lang].winFinal : langData[lang].win;
        } else {
            resultText.textContent = langData[lang].lose;
        }
    }
}
...
newGameBtn.addEventListener("click", () => {
    clearSave();
    gameScreen.style.backgroundImage = "url('graph/background1.png')";
...
continueBtn.addEventListener("click", () => {
    loadGame();
    if(game.currentQuestionIndex === 0 || game.currentQuestionIndex === 1){
        gameScreen.style.backgroundImage = "url('graph/background1.png')";
        enemyImg.src = "graph/enemy-skeleton.png";
    }
    if(game.currentQuestionIndex === 2){
    ...
    if(game.currentQuestionIndex === 3) initCircle4();
    startGame();
});
...
function initCircle4() {
    let points = [];
    let attempts = 0;
    while (points.length < 4 && attempts < 100) {
        attempts++;
        const p = randomCirclePoint(circle4Points);
        let valid = true;
        for(let exist of points){
            if(Math.hypot(p.x-exist.x, p.y-exist.y) < 50){
                valid = false;
                break;
            }
        }
    ...
    points.sort((a, b) => {
        const angleA = Math.atan2(a.y - circle4Points.O.y, a.x - circle4Points.O.x);
        const angleB = Math.atan2(b.y - circle4Points.O.y, b.x - circle4Points.O.x);
        return angleA - angleB;
    });
    circle4Points.A = points[0];
    circle4Points.B = points[1];
    circle4Points.C = points[2];
    circle4Points.D = points[3];
}
...
function checkGameOver() {
    if (game.enemyHp <= 0) { 
        ...
        if (game.currentQuestionIndex === 3) {
            ...
            restartBtn.textContent = langData[game.lang].backToHome;
            resultText.textContent = langData[game.lang].winFinal;
        } else {
            restartBtn.textContent = langData[game.lang].nextLevel;
    ...
    if (game.warriorHp <= 0) { 
        ...
        restartBtn.textContent = langData[game.lang].restart;
        resultModal.classList.remove("hidden"); 
        updateLanguageUI();
        clearSave(); 
    }
