...
def extract_content(file_path):
    ...
    # 提取游戏内容（从<div class="container">开始到</body>前）
    ...
    # 提取从container开始到</body>前的内容
    content_part = content[container_start:body_end]
    
    # 提取游戏内容（到第一个<script>标签前）
    script_start = content_part.find('<script>')
    if script_start != -1:
        game_content = content_part[:script_start].strip()
        # 提取JavaScript部分
        script_end = content_part.find('</script>', script_start)
        if script_end != -1:
            game_js = content_part[script_start:script_end+9].strip()
        else:
            game_js = ''
    else:
        game_content = content_part.strip()
        game_js = ''
